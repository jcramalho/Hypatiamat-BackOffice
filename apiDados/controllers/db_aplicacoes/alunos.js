...
Aluno.getAlunosFromTurma = function(turma, codprofessor){
    return new Promise(function(resolve, reject) {
        sql.query("Select id, user, numero, nome, datanascimento, escola, turma, email, confirmacao from alunos where turma=? and codprofessor=? Order by numero asc", [turma, codprofessor], function(err, res){
            if(err){
                console.log("erro: " + err)
                reject(err)
            }
