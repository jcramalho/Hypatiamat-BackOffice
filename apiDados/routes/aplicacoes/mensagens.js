...
router.get('/:idMensagem/vistos', passport.authenticate('jwt', {session: false}), function(req, res, next) {
    Mensagens.getQuemViuMensagem(req.params.idMensagem)
            .then(dados => res.jsonp(dados))
            .catch(erro => res.status(500).jsonp(erro))
});

router.get('/professores/:codigo', passport.authenticate('jwt', {session: false}), function(req, res, next) {
...
router.get('/alunos/:user/recentes', passport.authenticate('jwt', {session: false}), function(req, res, next) {
    Mensagens.getMensagensFromAlunoRecentes(req.params.user)
             .then(dados => res.jsonp(dados))
             .catch(erro => res.status(500).jsonp(erro))
});

router.get('/alunos/:user/antigas', passport.authenticate('jwt', {session: false}), function(req, res, next) {
...
router.get('/alunos/:user/number/naovistas', passport.authenticate('jwt', {session: false}), function(req, res, next) {
...
router.put('/:id/vista', passport.authenticate('jwt', {session: false}), function(req, res, next) {
